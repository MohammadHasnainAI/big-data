...
@st.cache_data
def load_data():
    # UPDATED: Use the 'Web Lite' CSV file (Small enough for GitHub)
    csv_file = "yelp_web.csv"
    
    try:
        df = pd.read_csv(csv_file)
    except FileNotFoundError:
        st.error(f"❌ Error: Could not find '{csv_file}'. Make sure you uploaded 'yelp_web.csv' to GitHub.")
        st.stop()
...
with st.sidebar:
    ...
    st.info(f"**Dataset:** Yelp Open Dataset (Web Lite)")
...
st.write("This system uses **Naive Bayes** trained on **real-world Yelp reviews** to detect sentiment.")
